...
Vector2I Application::get_position() const
{
    return Vector2I(sf::RenderWindow::getPosition());
}

Vector2UI Application::get_desktop_size() const
{
    Vector2UI size(sf::VideoMode::getDesktopMode().height, sf::VideoMode::getDesktopMode().width);
    return size;
}

Vector2UI Application::get_size() const
{
    return sf::RenderWindow::getSize();
}

void Application::set_view(const std::shared_ptr<IView>& view)
{
...
Vector2F Application::map_pixel_to_coords(const Vector2I& position)
{
    return sf::RenderWindow::mapPixelToCoords(position);
}

Vector2F Application::map_pixel_to_coords(int X, int Y)
{
